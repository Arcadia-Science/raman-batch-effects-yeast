...
def plot_ordinal_regression_results(y_true, y_pred, metrics_dict, ax=None):
    """
    Plot ordinal regression results as violin plots of predicted values at each true value.

    Arguments:
        y_true: True ordinal target values.
    ...
    data_for_violins = [y_pred[y_true == val] for val in unique_true]

    # Create violin plots.
    parts = ax.violinplot(
        data_for_violins,
        positions=unique_true,
        widths=0.5,
        showmeans=False,
        showmedians=True,
        showextrema=False,
    ...
    for pc in parts["bodies"]:
        pc.set_facecolor(DEFAULT_COLOR_PALETTE[0])
        pc.set_alpha(0.6)
    ...
    min_val = min(unique_true[0], y_pred.min())
    max_val = max(unique_true[-1], y_pred.max())
    ax.plot([min_val, max_val], [min_val, max_val], "k--", lw=1, alpha=0.5)

    ax.set_xlabel("True")
    ax.set_ylabel("Predicted")
    ax.set_xticks(unique_true)
